...
function extractRemedy(header: string): string | null {
  ...
  if (
    header.includes("Aconite_Napellus") ||
    header.includes("Aconite_napellus")
  )
    return "Aconite Napellus";
...
function cleanCategory(header: string): string {
  ...
  if (
    header.includes("Modality") ||
    header.includes("Worse") ||
    header.includes("Better")
  )
    return "Modalities";
...
async function main() {
  ...
  for (const file of files) {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      for (let j = 0; j < headers.length; j++) {
        ...
        const remedy =
          extractRemedy(headers[j]) || "General Homeopathic Knowledge";
  ...
  try {
    ...
    for (let i = 0; i < allDocs.length; i += batchSize) {
      ...
      console.log(
        `Ingested batch ${Math.floor(i / batchSize) + 1} of ${Math.ceil(allDocs.length / batchSize)}`,
      );
    }
